...
- [x-workspace-switcher](bin/x-workspace-switcher) Pops up a menu of workspace names to switch to; with an argument, renames the current workspace. Works on X11 only, as there is no support of Workspaces in Wayland, each compositor implement its own incompatible API, as always with this braindead system..
